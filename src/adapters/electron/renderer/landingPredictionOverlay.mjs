export function createLandingPredictionOverlay({ Leaflet, map }) {
  ...
  function render(prediction) {
    ...
    landingMarker.bindTooltip(buildLandingTooltip(prediction), {
      direction: 'top',
      offset: [0, -12]
    });
...
function buildLandingTooltip(prediction) {
  const eta = Number.isFinite(prediction?.etaSeconds)
    ? `${prediction.etaSeconds.toFixed(1)} s`
    : 'N/D';
  const confidence = typeof prediction?.confidence === 'string'
    ? prediction.confidence
    : 'low';

  return `Caida estimada | ETA ${eta} | Confianza ${confidence}`;
}
